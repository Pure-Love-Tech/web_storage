...
@section('title', admin_trans('Edit | ') . $storageProvider->name)
...
@section('content')
    <form id="vironeer-submited-form" action="{{ route('admin.settings.storage.update', $storageProvider->id) }}"
            ...
            <div class="card-body">
                <div class="vironeer-file-preview-box bg-light mb-3 p-4 text-center">
                    <div class="file-preview-box mb-3">
                        <img id="filePreview" src="{{ asset($storageProvider->logo) }}" height="100px" height="100px">
                    </div>
                </div>
        ...
        @if ($storageProvider->instructions)
            <div class="card mb-4">
                <div class="card-header">
                    <i class="far fa-question-circle me-2"></i>{{ $storageProvider->name . admin_trans(' Instructions') }}
                </div>
                <div class="card-body">
        ...
        @if ($storageProvider->credentials)
            <div class="card mb-4">
                <div class="card-header">
                    <i class="fa fa-key me-2"></i> {{ $storageProvider->name . admin_trans(' Credentials') }}
                </div>
                <div class="card-body">
    ...
    @if ($storageProvider->status)
        <div class="card">
            <div class="card-header">
                <i class="fas fa-hdd me-2"></i> {{ admin_trans('Test Connection') }}
            </div>
            <div class="card-body">
                <form action="{{ route('admin.settings.storage.test', $storageProvider->id) }}" method="POST">
                    @csrf
                    <button
                        class="vironeer-form-confirm btn btn-success">{{ admin_trans('Test ') . $storageProvider->name }}
                    </button>
                </form>
            </div>
        </div>
