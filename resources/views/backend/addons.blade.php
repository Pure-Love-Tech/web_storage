...
@section('title', admin_trans('Addons'))
                                ...
                                <div class="row g-3 align-items-center">
                                    <div class="col-md-8 col-xl-10">
                                        ...
                                        @else
                                            <h5 class="text-dark">{{ $addon->name }}</h5>
                                        @endif
                                        <span class="text-muted">{{ admin_trans('Version') . ':' . $addon->version }}</span>
                                    </div>
                                    <div class="col">
                                        <div class="row g-3">
                                            @if (!$addon->hasNoStatus())
                                                <div class="col-12">
                                                    <input id="addonStatus" type="checkbox" name="status"
                                                        data-toggle="toggle"
                                                        data-update-link="{{ route('admin.addons.update', $addon->id) }}"
                                                ...
                                                <div class="col-12">
                                                    <a href="{{ adminUrl($addon->action) }}"
                                                        class="btn btn-secondary w-100"><i
                                                            class="fa fa-cog me-2"></i>{{ admin_trans('Settings') }}</a>
                                                </div>
                                            @endif
                                        </div>
                                    </div>
    ...
    <div class="modal fade" id="uploadModal" tabindex="-1" aria-labelledby="uploadModalLabel" aria-hidden="true">
        <div class="modal-dialog modal-lg">
            <div class="modal-content">
                <h5 class="modal-header bg-primary text-white mb-0">{{ admin_trans('Upload an addon') }}</h5>
                <div class="modal-body p-4">
                    <div class="note note-warning">
                        ...
                        <ul class="mb-0">
                            ...
                            <li class="mb-0">
                                {{ admin_trans('Before uploading a new addon make sure to take a backup of your website files and database.') }}
                            </li>
                        </ul>
                    </div>
                    <form action="{{ route('admin.addons.upload') }}" method="POST" enctype="multipart/form-data">
                        @csrf
                        <div class="mb-3">
                            <label class="form-label">{{ admin_trans('Addon Purchase Code') }} </label>
                        ...
                        <div class="mb-4">
                            <label class="form-label">{{ admin_trans('Addon Files (Zip)') }} </label>
