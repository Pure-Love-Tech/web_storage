...
@section('title', admin_trans('Advertisements') . ' | ' . $advertisement->position)
...
@section('content')
    <form id="vironeer-submited-form" action="{{ route('admin.advertisements.update', $advertisement->id) }}" method="POST">
        ...
        <div class="card">
            ...
            <div class="card-body">
                <div class="mb-0">
                    <textarea id="html-editor" name="code" class="form-control" rows="10">{{ $advertisement->code }}</textarea>
                </div>
            </div>
        </div>
    </form>
