...
@section('title', admin_trans('Edit Payout Rate'))
@section('container', 'container-max-lg')
@section('back', route('admin.earnings.payout-rates.index'))
@section('content')
    <div class="card">
        <div class="card-body">
            <form id="vironeer-submited-form" action="{{ route('admin.earnings.payout-rates.update', $payoutRate->id) }}"
                method="POST" enctype="multipart/form-data">
                @csrf
                @method('PUT')
                <div class="vironeer-file-preview-box mb-3 bg-light p-4 text-center">
                    <div class="file-preview-box mb-3">
                        <img id="filePreview" src="{{ asset($payoutRate->flag) }}" class="rounded-3" width="100px"
                            height="100px">
                    </div>
                ...
                <div class="mb-4">
                    ...
                    <select name="country_id" class="form-select select2" required>
                        <option></option>
                        <option value="0" {{ !$payoutRate->country_id ? 'selected' : '' }}>
                            {{ admin_trans('** All Other Countries **') }}</option>
                        @foreach (countries() as $country)
                            <option value="{{ $country->id }}"
                                {{ $payoutRate->country_id == $country->id ? 'selected' : '' }}>
                                {{ $country->name }}</option>
                        @endforeach
                    </select>
                </div>
                <div class="mb-2">
                    ...
                    <div class="custom-input-group input-group">
                        <input type="text" name="amount" class="form-control input-price" placeholder="0.00"
                            value="{{ price($payoutRate->amount) }}" required>
