...
@section('title', admin_trans('Blog Comments'))
@section('content')
    <div class="card">
        <table id="datatable" class="table w-100">
            <thead>
                <tr>
                    ...
                    <th class="tb-w-7x">{{ admin_trans('Posted by') }}</th>
                    <th class="tb-w-20x">{{ admin_trans('Article') }}</th>
                    <th class="tb-w-3x">{{ admin_trans('Status') }}</th>
                    <th class="tb-w-7x">{{ admin_trans('Posted date') }}</th>
                    <th></th>
                </tr>
            </thead>
            <tbody>
                @foreach ($comments as $comment)
                    <tr class="item">
                        ...
                        <td>
                            ...
                            @else
                                <span class="text-muted">{{ admin_trans('Anonymous') }}</span>
                            @endif
                        </td>
                        <td><a href="{{ route('admin.blog.articles.edit', $comment->blogArticle->id) }}"
                                class="text-dark">{{ shortertext($comment->blogArticle->title, 30) }}</a>
                        </td>
                        <td>
                            @if ($comment->status == 0)
                                <span class="badge bg-warning">{{ admin_trans('Pending') }}</span>
                            @else
                                <span class="badge bg-success">{{ admin_trans('Published') }}</span>
                            @endif
                        </td>
                                ...
                                <ul class="dropdown-menu dropdown-menu-sm-end" data-popper-placement="bottom-end">
                                    <li>
                                        <a class="vironeer-view-comment dropdown-item" data-id="{{ $comment->id }}"
                                            href="#"><i class="fa fa-eye me-2"></i>{{ admin_trans('View') }}</a>
                                    </li>
                                    <li>
        ...
        <div class="modal-dialog">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title" id="viewCommentLabel">{{ admin_trans('View Comment') }}</h5>
                ...
                <div class="modal-footer">
                    ...
                    <form id="publishCommentForm" class="d-inline" action="#" method="POST">
                        ...
                        <button class="vironeer-form-confirm publish-comment-btn btn btn-success"><i
                                class="far fa-check-circle me-2"></i>{{ admin_trans('Publish') }}</button>
                    </form>
                </div>
            </div>
        </div>
