...
@section('content')
    ...
    <div class="card-v mb-3">
        <h5 class="mb-0">{{ translate('Subscription', 'settings') }}</h5>
        @if ($user->isSubscribed())
            <div class="form-section">
                @if (subscription()->is_expired)
                    <div class="alert alert-danger">
                        <i class="fa-solid fa-triangle-exclamation me-2"></i>
                        {{ translate('Your subscription has expired', 'settings') }}
                    </div>
                @endif
                <div class="subscription-box">
                    <div class="row g-3 align-items-center">
                        <div class="col-12 col-lg">
                            <h4 class="mb-3">{{ subscription()->plan->name }}</h4>
                            <p class="text-muted mb-0">{{ translate('Expiry date', 'settings') }} :
                                <span
                                    class="{{ subscription()->is_expired ? 'text-danger' : 'text' }}">{{ dateFormat($user->subscription->expiry_at) }}</span>
                            </p>
                        </div>
                        @if (subscription()->is_expired)
                            <div class="col-12 col-lg-auto">
                                <a href="{{ route('premium.index') }}" class="btn btn-outline-secondary btn-md w-100">
                                    <i class="fa-solid fa-rotate me-1"></i>
                                    <span>{{ translate('Renew', 'settings') }}</span>
                                </a>
                            </div>
                        @endif
                        <div class="col-12 col-lg-auto">
                            <form action="{{ route('user.settings.subscription.cancel') }}" method="POST">
                                ...
                                <button class="btn btn-danger btn-md w-100 action-confirm">
                                    ...
                                    {{ translate('Cancel Subscription', 'settings') }}
                                </button>
                            </form>
                        </div>
                    </div>
        ...
        @else
            <div class="card-body">
                <div class="py-5 text-center">
                    ...
                    <h4>{{ str(translate('You are currently in {plan_name} plan', 'settings'))->replace('{plan_name}', subscription()->plan->name) }}
                    </h4>
                    <p class="mb-3">
                        {{ str(translate('Settings subscription empty description', 'settings'))->replace('{plan_name}', subscription()->plan->name) }}
                    </p>
                    <a href="{{ route('premium.index') }}"
                        class="btn btn-primary">{{ translate('Upgrade Now', 'settings') }}</a>
                </div>
            </div>
        @endif
    </div>
    <div class="card-v mb-3">
        <h5 class="mb-0">{{ translate('Payment History', 'settings') }}</h5>
                    ...
                    <table class="table text-center table-borderless">
                        <thead>
                            <tr>
                                ...
                                <th>{{ translate('Status', 'settings') }}</th>
                                <th>{{ translate('Date', 'settings') }}</th>
                            </tr>
                        </thead>
                                ...
                                <tr>
                                    ...
                                    <td>
                                        @if ($transaction->interval == 1)
                                            {{ str(translate('{interval} day', 'settings'))->replace('{interval}', $transaction->interval) }}
                                        @else
                                            {{ str(translate('{interval} days', 'settings'))->replace('{interval}', $transaction->interval) }}
                                        @endif
                                    </td>
                                    <td>{{ $transaction->paymentGateway->name ?? '--' }}</td>
                                    <td>
                                        @if ($transaction->isPending())
                                            <span
                                                class="badge rounded-pill bg-orange">{{ translate('Pending', 'settings') }}</span>
                                        @elseif($transaction->isPaid())
                                            <span
                                                class="badge rounded-pill bg-green">{{ translate('Paid', 'settings') }}</span>
                                        @elseif($transaction->isCancelled())
                                            <span
                                                class="badge rounded-pill bg-red">{{ translate('Cancelled', 'settings') }}</span>
                                        @endif
                                    </td>
        ...
        @else
            <div class="py-5 text-center">
                ...
                <h4>{{ translate('You don\'t have any transactions', 'settings') }}</h4>
                <p class="mb-0">
                    {{ translate('No transactions description', 'settings') }}
                </p>
            </div>
        @endif
    </div>
