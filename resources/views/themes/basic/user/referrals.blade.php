...
@section('title', translate('Referrals', 'referrals'))
@section('content')
    <div class="card-v mb-3">
        <h4 class="mb-3">{{ translate('Invite your friends and earn money!', 'referrals') }}</h4>
        <p class="mb-3">
            {{ str(translate('Referral description', 'referrals'))->replace('{referral_percentage}', $settings->earnings->referrals->percentage) }}
        </p>
        <div class="col-lg-6 mb-3">
            <div class="input-group">
                <input id="refLink" type="text" class="form-control form-control-md radius radius-md"
                ...
                <button type="button" class="btn btn-secondary btn-md radius radius-md btn-copy"
                    id="input-group-button-right" data-clipboard-target="#refLink">
                    <i class="far fa-clone me-2"></i>{{ translate('Copy', 'referrals') }}
                </button>
            </div>
        </div>
        @include('themes.basic.partials.share-buttons', [
            'link' => route('home', 'ref=' . auth()->user()->username),
        ])
    </div>
    <div class="card-v">
        @if ($referrals->count() > 0)
            <h5 class="mb-3">{{ translate('Referrals', 'referrals') }}</h5>
            <div class="ref-table">
                <div class="dash-table">
                    <table class="table text-start table-borderless">
                        <thead>
                            <tr>
                                <th>{{ translate('Username', 'referrals') }}</th>
                                <th>{{ translate('Date', 'referrals') }}</th>
                                <th class="text-center">{{ translate('Earnings', 'referrals') }}</th>
                            </tr>
                        </thead>
                        <tbody class="text-muted">
                            @foreach ($referrals as $referral)
                                <tr>
                                    ...
                                    <td class="text-center">{{ earnings($referral->earnings) }}</td>
                                </tr>
                            @endforeach
                        </tbody>
                    </table>
        ...
        @else
            <div class="py-5 text-center">
                <div class="mb-3">
                    <svg xmlns="http://www.w3.org/2000/svg" width="200" height="200" viewBox="0 0 717.67004 628.75122"
                        ...
                        <path
                            ...
                            transform="translate(-228.12561 -94.1362)" fill="#e6e6e6" />
                        <polygon points="578.689 615.836 567.988 615.835 562.897 574.558 578.692 574.559 578.689 615.836"
                            fill="#a0616a" />
                        <path
                        ...
                        <path
                            ...
                            transform="translate(-228.12561 -94.1362)"
                            fill="{{ $themeSettings->colors->primary_color }}" />
                        <polygon points="149.919 613.706 162.661 613.705 168.724 564.551 149.916 564.552 149.919 613.706"
                            fill="#ffb6b6" />
                        <path
                ...
                <h4>{{ translate('You don\'t have any referrals', 'referrals') }}</h4>
                <p class="mb-0">
                    {{ translate('Share your referral link with your friends or via social media so that your referrals appear here.', 'referrals') }}
                </p>
            </div>
        @endif
    </div>
