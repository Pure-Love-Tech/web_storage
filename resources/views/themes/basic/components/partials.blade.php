@if (settings('popup')->status && !request()->hasCookie('popup_notice'))
...
@if (settings('actions')->gdpr_cookie_status && !request()->hasCookie('gdpr_cookie'))
    <div class="cookies">
        <div class="cookies-img"> <svg id="Capa_1" enable-background="new 0 0 80 80" height="80"
                viewBox="0 0 512 512" width="80" fill="{{ $themeSettings->colors->primary_color }}"
        ...
        <p class="cookies-text text-center my-3">{{ translate('gdpr_cookie_note') }}</p>
        <div class="d-flex justify-content-center">
            <button id="acceptCookie" class="btn btn-primary btn-md px-5 me-3">{{ translate('Got it') }}</button>
            @if (settings('general')->terms_of_service_link)
                <a class="btn btn-outline-secondary btn-md px-5"
                    href="{{ settings('general')->terms_of_service_link }}">{{ translate('More') }}</a>
            @endif
        </div>
    </div>
@endif
