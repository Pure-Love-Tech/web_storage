...
function purchaseCodeValidation($purchaseCode, $alias)
{
    try {
        $client = new Client();
        $res = $client->get(config('vironeer.api.license') . '?purchase_code=' . $purchaseCode . '&alias=' . strtolower($alias) . '&website=' . url('/'));
...
function filePermissions()
{
    $filePermissions = [
        ...
        base_path('lang/'),
        base_path('lang/themes/'),
        base_path('lang/themes/basic/en/'),
        base_path('resources/'),
